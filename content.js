  ...
  async function scrapeSkillsFast() {
    ...
    try {
      ...
      const skillsSection = (function(){
        ...
        for (const sec of sections) {
          const h2 = q('h2, h3', sec);
          if (h2 && /skills/i.test(h2.textContent || '')) return sec;
        }
        return null;
      })();
      if (skillsSection) {
        qa('li, .pvs-list__paged-list-item, .artdeco-list__item', skillsSection).forEach((li) => {
          ...
          const name = (function cleanSkill(v){
      ...
      // Inline bullets near the top card
      qa('.t-14.t-normal .display-flex.align-items-center span[aria-hidden="true"], .display-flex.full-width .t-14.t-normal.t-black.display-flex.align-items-center span[aria-hidden="true"]').forEach((el) => {
        splitSkillsInline(el.textContent).forEach(s => set.add(s));
      });
    } catch {}
    return Array.from(set).slice(0, 5);
  }

  // Fast contact: harvest from current DOM only (no overlay fetch)
  async function scrapeContactInfoFast(includeContact) {
    ...
    try {
      ...
      const harvestFromRoot = (root) => {
        ...
        // Emails
        try {
          const emailSpans = qa('a[href^="mailto:"], a[href*="@"], span, div, p', scope).slice(0, 600);
      ...
      // Prefer explicit contact containers near top card
      const roots = [
        q('.pv-top-card'),
        q('[data-test-id="hero-summary-card"]'),
        q('main')
      ].filter(Boolean);
      ...
      let profileSlug = '';
      try {
        const m = (location.href || '').match(/\/in\/([^\/]+)\/?/i);
        if (m && m[1]) profileSlug = m[1].toLowerCase();
      } catch {}
      data.websites = uniq(data.websites)
